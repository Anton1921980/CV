...
function send(event, php) {
    ...
    req.onload = async function() {
        ...
        if (req.status >= 200 && req.status < 400) {
            ...
            // console.log("TCL: req.status", req.status);
            // console.log("TCL: this.response", this.response);

            json = await JSON.parse(this.response); // Ебанный internet explorer 11
...
for (var i = 0, count = arrInput.length; i < count; i++) {
    arrInput[i].addEventListener("focus", function() {
        this.nextElementSibling.classList.add("write");
    });
    arrInput[i].addEventListener("blur", function() {
        if (this.value == false)
            this.nextElementSibling.classList.remove("write");
    });
}
